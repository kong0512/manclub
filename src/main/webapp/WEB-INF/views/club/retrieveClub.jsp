...
<body>
	...
	<h1>동아리 조회</h1>
		...
		<tr>
		<td>${thisclub.club_name }</th>
		<td colspan="2">${thisclub.club_information }</th>
		<td>
			<c:choose>
		...
		<td><a href="${pageContext.request.contextPath}/club/manageClub/${clubID}">
		<button class="btn btn-primary btn-sm">동아리 관리</button>
		</a>
		</td>
		</tr>
		</table>
			...
			<td>
				<c:forEach var="gradetest" items="${joinedlist}">
					<c:if test="${student.studentID == gradetest.studentID}" >
   						<c:choose>
   							<c:when test="${gradetest.member_grade == 0}">대기중</c:when>
							<c:when test="${gradetest.member_grade == 1}">회원</c:when>
